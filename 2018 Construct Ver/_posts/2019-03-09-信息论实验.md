...
首先，这里是我基于 Matlab 程序改进成的 Python 程序，是个**错误示例**，其原因在于 Matlab 中对矩阵操作转移到 Python 中 DataFrame 实现起来有点麻烦（也可能是迫于对 Numpy 用的不熟练），DataFrame 多用于神经网络数据导入是非常方便的，具体分隔来处理有点麻烦。
